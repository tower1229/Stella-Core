#!/usr/bin/env node
/**
 * Record synthetic_contract DeliveryEvidence for Issue #12 declared-scope discovery.
 * Writes result "implemented" only — does not claim Exact Host / real_main verified.
 *
 * Usage:
 *   npm run build && node scripts/compile.mjs test
 *   node scripts/record-declared-scope-discovery-evidence.mjs --evidence-directory /path/to/private-evidence
 */
...
const TARGETS = ["07", "M-04", "M-06", "G-10"];
...
const modules = [
  path.join(root, "dist/src/canghai/declared-scope-discovery.js"),
];
const testEntry = path.join(root, ".test-dist/tests/declared-scope-discovery.test.js");
try {
...
const { stderr: testErr } = await execFileAsync(
  ...
  throw new Error("synthetic_declared_scope_discovery_failed");
});
...
const version = {
  ...
  source: hash("stella-core:issue-12:declared-scope-discovery"),
  profile: hash("synthetic-profile:declared-scope-discovery"),
  policy: hash("synthetic-policy:declared-scope-discovery"),
  configuration: hash("synthetic-config:declared-scope-discovery"),
  model: hash("synthetic-model:structured-clue-selection"),
...
const summary = {
  schemaVersion: "stella.declared-scope-discovery-evidence/v1",
  issue: 12,
  ...
  syntheticHarness: "tests/declared-scope-discovery.test.ts",
};
...
process.stdout.write(`${JSON.stringify({
  schemaVersion: "stella.declared-scope-discovery-evidence-report/v1",
  environment: ENVIRONMENT,
  result: "implemented",
