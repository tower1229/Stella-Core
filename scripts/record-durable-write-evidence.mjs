#!/usr/bin/env node
/**
 * Record synthetic_contract DeliveryEvidence for Issue #9 durable-write targets.
 * Writes result "implemented" only — does not claim Exact Host / real_main verified.
 *
 * Usage:
 *   npm run build && npm run test --silent -- managed-durable-write  # or compile first
...
const TARGETS = ["04", "G-05", "G-06", "G-09", "M-11"];
...
const managedModule = path.join(root, "dist/src/canghai/managed-durable-write.js");
const testEntry = path.join(root, ".test-dist/tests/managed-durable-write.test.js");
try {
  await readFile(managedModule);
...
const { stdout: testOutput, stderr: testErr } = await execFileAsync(
  ...
  throw new Error("synthetic_managed_durable_write_failed");
});
...
const version = {
  core,
  sourceClean: status === "",
  artifact: hash(await readFile(managedModule)),
  host: hash(hostSdk),
  ...
  source: hash("stella-core:issue-9:managed-durable-write"),
  profile: hash("synthetic-profile:managed-durable-write"),
  policy: hash("synthetic-policy:managed-durable-write"),
  configuration: hash("synthetic-config:managed-durable-write"),
...
const summary = {
  schemaVersion: "stella.durable-write-evidence/v1",
  issue: 9,
  ...
  publicSeam: [
    ...
    "GitCangHaiDurability",
    "MemoryTransaction",
  ],
  syntheticHarness: "runManagedDurableRecord",
};
...
process.stdout.write(`${JSON.stringify({
  schemaVersion: "stella.durable-write-evidence-report/v1",
  environment: ENVIRONMENT,
  result: "implemented",
