#!/usr/bin/env node
/**
 * Record synthetic_contract DeliveryEvidence for Issue #18 processing / output authorization.
...
const TARGETS = ["13", "21", "G-08", "G-10", "I-10", "C-07"];
...
const modules = [
  path.join(root, "dist/src/openclaw/turn-audience.js"),
  path.join(root, "dist/src/openclaw/processing-authority.js"),
  path.join(root, "dist/src/canghai/source-policy.js"),
  path.join(root, "dist/src/plugin.js"),
];
const testEntry = path.join(root, ".test-dist/tests/processing-authority.test.js");
try {
...
const { stdout: testOutput, stderr: testErr } = await execFileAsync(
  ...
  throw new Error("synthetic_processing_authority_failed");
});
...
const version = {
  ...
  source: hash("stella-core:issue-18:processing-authority"),
  profile: hash("synthetic-profile:processing-authority"),
  policy: hash("synthetic-policy:processing-authority"),
  configuration: hash("synthetic-config:processing-authority"),
...
const summary = {
  schemaVersion: "stella.processing-authority-evidence/v1",
  issue: 18,
  environment: ENVIRONMENT,
  result: "implemented",
  note: "synthetic_contract only; implemented ≠ verified; Exact Host / real_main / native Codex not claimed. G-08/G-10/I-10/C-07 = processing-authority audience and stage slice only. Work 21 = Core audience/quote boundary slice shared with later Host-memory tickets.",
  targets: TARGETS,
  recordedAt,
  testSummary: testOutput.trim().split("\n").slice(-20),
  publicSeam: [
    "resolveTurnAudience",
    "assertPrivateContextAudience",
    "bindProcessingAuthority",
    "assertProcessingAuthority",
    "assertProcessingStage",
    "assertPurposeAxes",
    "assertQuoteCapability",
    "StellaInitializer.bindRun",
    "StellaInitializer.assertRun",
    "StellaInitializer.revokeActiveRuns",
    "preparePersonalViews.audience",
    "plugin.before_prompt_build",
  ],
  syntheticHarness: "tests/processing-authority.test.ts",
};
...
try {
  ...
  if (!(error && typeof error === "object" && "code" in error && error.code === "ENOENT")) {
    // keep empty manifest for first write
  }
}
...
process.stdout.write(`${JSON.stringify({
  schemaVersion: "stella.processing-authority-evidence-report/v1",
  environment: ENVIRONMENT,
  result: "implemented",
