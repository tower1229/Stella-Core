#!/usr/bin/env node
/**
 * Record synthetic_contract DeliveryEvidence for Issue #10 revoke / late-delivery targets.
...
const TARGETS = ["06", "G-05", "G-08", "I-07", "I-08", "I-12", "C-08"];
...
const modules = [
  path.join(root, "dist/src/openclaw/run-admission.js"),
  path.join(root, "dist/src/openclaw/host-admission-isolation.js"),
  path.join(root, "dist/src/openclaw/initialization.js"),
];
const testEntry = path.join(root, ".test-dist/tests/run-admission.test.js");
try {
...
const { stdout: testOutput, stderr: testErr } = await execFileAsync(
  ...
  throw new Error("synthetic_revoke_late_delivery_failed");
});
...
const version = {
  ...
  source: hash("stella-core:issue-10:revoke-late-delivery"),
  profile: hash("synthetic-profile:revoke-late-delivery"),
  policy: hash("synthetic-policy:revoke-late-delivery"),
  configuration: hash("synthetic-config:revoke-late-delivery"),
...
const summary = {
  schemaVersion: "stella.revoke-late-delivery-evidence/v1",
  issue: 10,
  environment: ENVIRONMENT,
  result: "implemented",
  note: "synthetic_contract only; implemented ≠ verified; Exact Host / real_main not claimed. G-08/C-08 = old-run and late-delivery slice only. I-12 = start-failure Host-config isolation + unload-observable deny without hooks.",
  targets: TARGETS,
  recordedAt,
  ...
  publicSeam: [
    ...
    "persist.assertRun",
    "withFinalValidation.assertRun",
    "reply_payload_sending.assertRun",
    "isolateHostProfile",
    "assertHostProfileIsolated",
    "registerStellaInitialization",
  ],
  syntheticHarness: "tests/run-admission.test.ts",
};
...
process.stdout.write(`${JSON.stringify({
  schemaVersion: "stella.revoke-late-delivery-evidence-report/v1",
  environment: ENVIRONMENT,
  result: "implemented",
