#!/usr/bin/env node
/**
 * Record synthetic_contract DeliveryEvidence for Issue #11 install / manual init targets.
...
const TARGETS = ["31", "I-01", "I-02", "I-03", "I-05", "C-01", "C-09"];
...
const { stdout: testOutput, stderr: testErr } = await execFileAsync(
  ...
  throw new Error("synthetic_install_init_failed");
});
...
const version = {
  ...
  source: hash("stella-core:issue-11:install-init"),
  profile: hash("synthetic-profile:install-init"),
  policy: hash("synthetic-policy:install-init"),
  configuration: hash("synthetic-config:install-init"),
...
const summary = {
  schemaVersion: "stella.install-init-evidence/v1",
  issue: 11,
  ...
  publicSeam: [
    ...
    "StellaInitializer.initialize",
    "verifyHost",
    "verifyInitializationContext",
    "compileInitializationSource",
    "/stella-initialize",
    "stella_initialize",
    "stella.initialize",
  ],
...
process.stdout.write(`${JSON.stringify({
  schemaVersion: "stella.install-init-evidence-report/v1",
  environment: ENVIRONMENT,
  result: "implemented",
