...
test("plugin stages prediction before finalization, learns from a tool outcome, and retrieves it", async () => {
  ...
  try {
    ...
    const finalizeContext = { ...firstContext, runId: "host-finalize-run" };
    await requireHook(hooks, "before_agent_finalize")(
      {
        runId: finalizeContext.runId,
...
test("target agent passes the gate and ordinary turn bypasses Cortex context", async () => {
  ...
  try {
    ...
    assert.ok("appendContext" in prompt && typeof prompt.appendContext === "string");
...
test("semantic routing failure is explicit and does not masquerade as ordinary", async () => {
  ...
  try {
    ...
    assert.deepEqual(errors, [
      "Stella semantic routing failed: invalid_model_route:route_shape",
    ]);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
