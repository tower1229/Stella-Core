...
export async function prepareQuestionEvidence(input: {
  ...
  const prompt = [
    ...
    retrieval ? "This adapter performs multi-round semantic selection over every descriptor page of one configured catalog. Only selected originals enter this assessment. Some Host originals were also read to construct descriptors, as recorded in retrievalCoverage; these reads are not independent support. Unselected descriptors and access exclusions cannot prove absence. It does not prove all personal files or Host sessions were searched." : "This adapter reads the eligible originals of one explicitly configured Alpha catalog. It does not prove all personal files or Host sessions were searched. Empty catalog or incomplete/declared-subset coverage is not proof an event never happened.",
