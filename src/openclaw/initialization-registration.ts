  ...
  const verifyHost = async (materialization: Materialization, host?: { setup: true }) => {
    ...
    for (const file of materialization.files.filter((file) => !file.target.startsWith("skills/"))) {
      ...
      if (!isRecord(observed) || !isRecord(observed.file) || typeof observed.file.content !== "string") {
        throw new InitializationError("host_projection_mismatch");
      }
      if (file.target === "IDENTITY.md") {
        assertHostChannelIdentity(await readIdentity(), observed.file.content);
      }
      if (bytesVersion(observed.file.content) !== file.sha256) throw new InitializationError("host_projection_mismatch");
    }
