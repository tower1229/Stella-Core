...
2026-09-15 全流程处理授权（Issue #18 / T12）：`resolveTurnAudience` 在加载 PCA／私人视图前按 Host 绑定身份与 sessionKey（子 Agent／cron）判定受众；仅 `owner_direct` 允许私人上下文。`bindProcessingAuthority` 冻结用户／受众／用途／模型／会话／run／deployment／Memory Generation；`assertProcessingStage` 对 read／derive／learn／quote／deliver 分阶段校验，可读不等于可推导／引用／投递。父任务摘要不能扩大子 Agent 权限。账本仅 `synthetic_contract`＋`implemented`，不等于 Exact Host／real_main／native Codex verified。
