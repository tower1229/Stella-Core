...
CatalogEntry 指向的是持久描述对象；removed Source 描述仅保留无原文的身份、版本、移除状态及原件缺失信息，locator 本身仍须存在。active 对象的依赖必须可解析到允许使用的版本。历史记录依赖被移除时保留不可用于当前推理的标记，不用死链接或偷偷补回原文满足校验。
...
| `capabilities` | `{ id, required, adapter_id, adapter_version, config_ref, acceptance_ref }[]` |
